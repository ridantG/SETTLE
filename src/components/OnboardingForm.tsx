...
export default function OnboardingForm({ user, profileData, onSave, isSaving }: OnboardingFormProps) {
    ...
    // Manage internal state, converting numbers to strings for form inputs.
    const [formData, setFormData] = useState({
        ...profileData,
        age: profileData.age?.toString() || ''
    });

    // Keep form in sync if initial data from parent page changes.
    useEffect(() => {
        setFormData({
            ...profileData,
            age: profileData.age?.toString() || ''
    ...
    const handleHabitChange = (habit: keyof Profile, value: boolean) => {
        setFormData(prev => ({ ...prev, [habit]: value } as any));
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Convert the form's string 'age' back into a proper number.
            ...
            <fieldset disabled={isSaving || isSummarizing}>
                ...
                {currentStep === 3 && (
                    <div className="space-y-6">
                        ...
                        <div>
                            <button type="button" onClick={handleSummarize} disabled={isSummarizing}>Summarize with AI</button>
                        </div>
                        {formData.role === 'lister' && (
                            <div>
                                ...
                                <MultiImageUpload
                                    user={user}
                                    bucket="flat-photos"
                                    initialImageUrls={formData.flat_image_urls}
                                    // FIX: never return null, always return the updated object
                                    onUploadSuccess={(urls) => setFormData(p => ({...p, flat_image_urls: urls}))}
                                />
                            </div>
                        )}
                    </div>
