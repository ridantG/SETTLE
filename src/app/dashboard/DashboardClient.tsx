...
const FeatureCard = ({ onClick, title, description, icon }: { onClick: () => void, title: string, description: string, icon: React.ReactNode }) => ( <div onClick={onClick} className="bg-white p-8 rounded-2xl shadow-lg hover:shadow-2xl hover:scale-105 transition-transform duration-300 group h-full flex flex-col text-center cursor-pointer"><div className="mx-auto mb-4">{icon}</div><h3 className="text-2xl font-bold text-gray-900 group-hover:text-green-600 transition-colors">{title}</h3><p className="mt-2 text-gray-500 flex-grow">{description}</p></div> );
...
export default function DashboardClient({ user, profile }: DashboardClientProps) {
    ...
    const handleNavigation = async (role: 'seeker' | 'lister', destination: string) => {
        ...
        // This 'update' can now be called with confidence, because the server-side
        // 'upsert' has guaranteed that a profile row exists.
