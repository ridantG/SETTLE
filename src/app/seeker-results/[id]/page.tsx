                 ...
                 <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                    <div className="grid grid-cols-1 md:grid-cols-3">
                        <div className="p-8 md:col-span-1 flex flex-col items-center justify-around bg-gray-50 space-y-6">
                            <div className="text-center"><img src={profile.image_url || 'https://placehold.co/150x150'} alt={profile.name || 'User profile'} className="w-36 h-36 rounded-full object-cover border-4 border-white shadow-lg"/><h1 className="text-3xl font-bold text-gray-800 mt-4">{profile.name}, {profile.age}</h1><p className="text-gray-500 text-md mt-1">{profile.organization}</p></div>
                            <CompatibilityScore score={compatibilityScore} />
                        </div>
                        <div className="p-8 md:col-span-2">
                            <h2 className="text-2xl font-bold text-gray-800">About {profile.name || 'Them'}</h2><p className="text-gray-600 mt-2 whitespace-pre-wrap">{profile.description || "No description provided."}</p>
                            <div className="grid grid-cols-2 gap-4 mt-6 text-sm"><p><strong className="text-gray-500 block">Diet:</strong> {profile.diet || 'N/A'}</p><p><strong className="text-gray-500 block">Drinks:</strong> {profile.drinks ? 'Yes' : 'No'}</p><p><strong className="text-gray-500 block">Smokes:</strong> {profile.smokes ? 'Yes' : 'No'}</p><p><strong className="text-gray-500 block">Pets:</strong> {profile.has_pets ? 'Yes' : 'No'}</p></div>
                        </div>
                    </div>
                    {profile.flat_image_urls?.length > 0 && (<div className="p-8 border-t border-gray-200"><h2 className="text-2xl font-bold text-gray-800 mb-4">The Space</h2><div className="grid grid-cols-2 md:grid-cols-3 gap-4">{profile.flat_image_urls.map((url: string, index: number) => (<img key={index} src={url} alt={`Flat photo ${index + 1}`} className="w-full h-40 object-cover rounded-lg shadow-md"/>))}</div></div>)}
                    <div className="bg-gray-50 p-6 border-t flex flex-col sm:flex-row justify-center items-center gap-4"><button onClick={handleLike} disabled={isProcessing} className="w-full sm:w-auto px-8 py-3 bg-green-500 text-white font-semibold rounded-full hover:bg-green-600 disabled:opacity-50">Show Interest</button><button onClick={handleReportUser} disabled={isProcessing} className="text-sm text-gray-500 hover:text-red-600 hover:underline disabled:opacity-50">Report this user</button></div>
                 </div>
            </main>
        </div>
    );
